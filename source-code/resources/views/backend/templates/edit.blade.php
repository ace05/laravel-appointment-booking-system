...
@section('title')
   {{{  __('site_email_templates') }}}
@endsection
@section('content')
<div class="app-title">
    <div>
        <h1><i class="icon fa fa-chevron-right"></i> {{{  __('site_email_templates') }}} - {{{ $template->name }}}</h1>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12">
      <div class="tile">
        <div class="tile-body">
            {!! Form::open(['url' => route('admin.edit.email.template', ['slug' => $template->slug])]) !!}
                <div class="form-group">
                    {!! Form::label('subject', __('site_email_subject') ,['class' => 'control-label']); !!}
